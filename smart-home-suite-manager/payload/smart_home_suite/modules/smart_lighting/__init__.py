"""Smart Lighting module bundled in Smart Home Suite."""
...
STORAGE_KEY = "smart_lighting_panel"
STORAGE_VERSION = 1
PANEL_PATH = "lighting"
...
async def async_setup_module(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Register storage API, static frontend and panel."""
    data = _data(hass)
    ...
    if frontend.async_panel_exists(hass, PANEL_PATH):
        ...
        if existing and existing.get("component_name") != "custom":
            _LOGGER.error("Cannot register Smart Lighting: panel path /%s is already in use", PANEL_PATH)
            return False
        try:
    ...
    await panel_custom.async_register_panel(
        ...
        module_url=f"{STATIC_URL}/{FRONTEND_FILE}?v={MODULE_VERSION}-suite020",
        require_admin=False,
...
async def async_unload_module(hass: HomeAssistant, entry: ConfigEntry) -> None:
    """Remove the panel while keeping persisted module configuration."""
...
async def websocket_get_config(hass, connection, msg) -> None:
    """Return saved Smart Lighting configuration."""
...
@websocket_api.websocket_command(
    {vol.Required("type"): f"{LEGACY_DOMAIN}/config/save", vol.Required("config"): dict}
)
